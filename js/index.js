...
(function(){
  ...
  // 渲染到页面 把生成的5个小li字符串给 sub dom盒子
  $(".province .sup").html(dataLi)

  // 鼠标经过tab栏切换 .on动态绑定 鼠标经过当前的小li要高亮显示
  ...
  function render(currentEle) {
    ...
    let dataSub = ""
    $.each(hotData[currentEle.index()].brands, function(index,ele){
      dataSub += `
        <li><span>${ele.name}</span>${ele.num}<span> <s class=${ele.flag?"icon-up":"icon-down"}></s></span></li>
      `
    })
    console.log(dataSub)
    // 渲染到页面
    $(".province .sub").html(dataSub)
  }
